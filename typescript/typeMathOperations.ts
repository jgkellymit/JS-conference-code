class parseOrderOfOperations{
  ourArguments:Array<string>;
  stringToCalculate:string;
  results:number;

  constructor (array_from_commandline:Array<string>){
  ...
  doMath(array:Array<string>):number {
    var first_number = "";
    const output = -1;
    for(let index = 0; index < array.length; index ++){
      ...
      if (value === "+"){
        ...
        var evaluated:Array<any> = this.calcExp(first_number, array.slice(index + 1));
        if (evaluated[1] == 0){
          return evaluated[0];
        }
  ...
  calcMult(first_number:string, array:Array<string>): number{
    var second_number = "";
    for(let index = 0; index < array.length; index ++){
      const value = array[index];
  ...
  calcDiv(first_number:string, array:Array<string>): number{
    var second_number = "";
    for(let index = 0; index < array.length; index ++){
      const value = array[index];
      if (second_number === "" && value === "-"){ //to handle negative numbers
  ...
  calcExp(first_number:string, array:Array<string>): Array<any>{
    var second_number = "";
    for(let index = 0; index < array.length; index ++){
      const value = array[index];
      if (second_number === "" && value === "-"){ //to handle negative numbers
  ...
  checkValidCharacter(character:string):boolean{
    var valid_character = ['1','2','3','4','5','6','7','8','9','0','#','-','+','/','*', ' '];
    if (!(character in valid_character)){
